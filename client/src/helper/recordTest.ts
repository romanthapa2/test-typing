...
//     setTypedWord,
// } from "store/actions";
import { appendTypedHistory,backtrackWord,setChar } from "../store/wordSlice";
import { store } from "../store/store";
import useResetTest from "./resetTest";
import useStartTimer from "./startTImer";
...
export const recordTest = (key: string, ctrlKey: boolean) => {
    ...
    if (!timerId && key !== "Tab") useStartTimer();
    const currWordEl = activeWordRef?.current!;
